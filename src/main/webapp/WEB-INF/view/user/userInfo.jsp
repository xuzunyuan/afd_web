					...
					<div class="memb-main">
						<div class="memg-bd personaldata">
							<div class="buyerInfo">
								...
								<div class="editdata">
									<form class="form" id="form" action="${ctx}/user/saveUserInfo.action" method="post">
										...
										<fieldset>
											<div class="formGroup">
												...
												<div class="form-item">
													...
													<div class="item-cont">
														<input type="text" name="nickname" value='<c:out value="${user.nickname}" />' class="txt lg w-xl" placeholder="15个字以内的字母、数字、汉字"><span id="errMsg1" class="note errTxt hide">15个字以内的字母、数字、汉字。</span>
													</div>
												</div>
												<div class="form-item">
													<div class="item-label"><label>真实姓名：</label></div>
													<div class="item-cont">
														<input type="text" name="userExt.realName" value='<c:out value="${user.userExt.realName}" />' class="txt lg w-xl" placeholder="15个字以内的字母、汉字"><span id="errMsg2" class="note errTxt hide">15个字以内的字母、汉字。</span>
													</div>
												</div>
												<div class="form-item">
													<div class="item-label"><label>性别：</label></div>
													<div class="item-cont">
														<label><input type="radio" name="userExt.gender" class="radio" value="" <c:if test="${empty(user.userExt) || empty(user.userExt.gender) }" >checked="checked"</c:if >>保密</label>
														<label><input type="radio" name="userExt.gender" class="radio" value="0" <c:if test="${!empty(user.userExt) && user.userExt.gender =='0' }" >checked="checked"</c:if >>男</label>
														<label><input type="radio" name="userExt.gender" class="radio" value="1" <c:if test="${!empty(user.userExt) && user.userExt.gender =='1' }" >checked="checked"</c:if >>女</label>
													</div>
												</div>
												<div class="form-item">
													<div class="item-label"><label>生日：</label></div>
													<div class="item-cont">
														<input type="text" id="birthday"  class="txt" readonly="readonly" name="userExt.birthday" value="<fmt:formatDate value="${user.userExt.birthday}" pattern="yyyy-MM-dd" />" onClick="WdatePicker()" />
													</div>
												</div>
												<div class="form-item">
													<div class="item-label"><label>星座：</label></div>
													<div class="item-cont">
														<input type="text" disabled="disabled" class="txt" readonly="readonly" id="constellation" value="${user.userExt.constellation}" />
														<input type="hidden" name="userExt.constellation" value="${user.userExt.constellation}" />
													</div>
												</div>
												<div class="form-item">
													<div class="item-label"><label>居住地：</label></div>
													<div class="item-cont">
														<input type="text" name="userExt.homeAddress" value='<c:out value="${user.userExt.homeAddress}" />' class="txt lg w-xl" placeholder="60个字以内"><span id="errMsg3" class="note errTxt hide ">60个字以内</span>
													</div>
												</div>
												<div class="form-item">
													...
													<div class="item-cont">
														...
														<input name="save" type="button" value="保 存" class="btn btn-primary xl">
													</div>
												</div>
											</div>
										</fieldset>
